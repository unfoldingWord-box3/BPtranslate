...
Fresh git worktree: run `scripts/worktree-init.ps1` (Windows) or `scripts/worktree-init.sh` (Linux — Ubuntu has no `pwsh`) from the worktree root. Both copy `api/.dev.vars` from main, run a real (cache-fast) `npm install`, build `web/dist` (else `wrangler dev`'s `[assets]` aborts and `npm run dev` fails), and apply the local D1 migrations (else `POST /api/auth/dev` 500s with `no such table: users`); each step is idempotent and skips itself when its output already exists. The `.sh` also rebuilds graft's index when this is a graft-wired checkout (best-effort). **Teardown: always use `scripts/worktree-cleanup.ps1` (dry-run by default; `-Remove '<path>'` to delete one). Never `rm -rf` / `Remove-Item -Recurse` a worktree by hand.** The init script no longer junctions `node_modules` from main: junctions were a Windows footgun — a recursive delete of a worktree followed the junction and wiped main's `node_modules` and (via npm's `@bible-editor` workspace links) main's `web/`+`api/` source. `worktree-cleanup.ps1` safely unlinks any leftover junctions (link only, never the target) before deleting.
...
`scripts/worktree-cleanup.ps1` and `scripts/worktrees.ps1` are thin forwarders — the real implementations live once in `C:\GH\dotfiles\windows\` and take `-RepoPath`, so every repo on the machine shares one copy. (`scripts/worktree-init.ps1` and `scripts/worktree-init.sh` are **not** forwarders — they are full copies that live in this repo; the dotfiles `worktree-init.ps1` still differs, e.g. it runs `graft build` but has no `.dev.vars` copy. Reconciling the two `.ps1` copies is a dotfiles issue, out of scope here.) This repo's copy had drifted and was the *stale* side, missing dubious-ownership handling, per-worktree try/catch, and the orphan ownership guard. The drift also hid a bug in which a **failed** `git status` was read as a clean working tree, so a worktree holding uncommitted work could classify SAFE and be deleted. To see every repo's worktrees at once, run `C:\GH\dotfiles\windows\Sweep-Worktrees.ps1`.
